    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('authenticate', (data) => {
        try {
          ...
          console.log(`🔍 Received socket authentication token: "${token}"`);

          // Handle demo token for testing
          if (token === 'tourist-demo-token') {
            console.log('✅ Demo token recognized, authenticating...');
            socket.userId = 'demo_tourist_123';
            socket.userType = 'tourist';
            socket.digitalId = 'DEMO-V-ID-123';
